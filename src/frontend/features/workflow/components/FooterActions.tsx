// FooterActions — the dashboard's footer fragment. Its content depends on
// the active content tab:
//
//   - PROMPT / JSON — a "#N" button per spawned pod (queue another
//     generation on an existing pod, skipping pod creation) plus the
//     primary Generate button (spawns a fresh pod).
//   - OUTPUT — a List | Thumbs view toggle for the generations pane
//     (generate controls don't apply while browsing results).
//
...
export type FooterActionsProps = {
    ...
    onGenerate: () => void;
    /** Active content tab — pod/Generate controls show for PROMPT and
        JSON only; the OUTPUT tab swaps them for the view toggle. */
...
export const FooterActions: React.FC<FooterActionsProps> = ({
    ...
    // OUTPUT tab — the List | Thumbs view toggle replaces the generation
    // controls entirely (there's nothing to generate while browsing
    // results).
    if (contentTab === 'results') {
        return (
            <div style={{ display: 'flex', flexDirection: 'row', alignItems: 'center', gap: 8 }}>
                <div style={{ flex: '1 1 auto' }} />
                {(['list', 'thumbs'] as const).map((mode) => {
                    const active = outputView === mode;
                    return (
                        <Btn
                            key={mode}
                            className={active ? undefined : 'sg-hover'}
                            onClick={() => onOutputViewChange(mode)}
                            title={mode === 'list' ? 'List view' : 'Thumbnail grid view'}
                            data-testid={`output-view-${mode}`}
                            style={{
                                padding: '3px 10px',
                                ...
                                backgroundColor: active ? theme.accentSoft : theme.surface1
                            }}
                        >
                            {mode === 'list' ? 'List' : 'Thumbs'}
                        </Btn>
                    );
                })}
            </div>
    ...
    // PROMPT / JSON tabs — the pod buttons + Generate.
    return (
