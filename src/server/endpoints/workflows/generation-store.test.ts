...
function seedEntry(result: GenerationResultItem[]): void {
    fs.mkdirSync(path.dirname(assetsDir()), { recursive: true });
    writeGenerationFile(tmpRoot, WORKFLOW_ID, GENERATION_ID, makeEntry(result));
}
...
describe('persistResultAssets', () => {
    it('writes data: payloads to index-named asset files and returns file: references', () => {
        ...
        const persisted = persistResultAssets(tmpRoot, WORKFLOW_ID, GENERATION_ID, results);
    ...
    it('normalizes the mimeType from the data: meta when the item lacks one', () => {
        const results: GenerationResultItem[] = [
    ...
    it('leaves remote http(s) and already file-backed items untouched', () => {
        const results: GenerationResultItem[] = [
    ...
    it('keeps a malformed data: payload inline rather than writing garbage', () => {
        const results: GenerationResultItem[] = [
...
describe('deleteGenerationFiles', () => {
    it('removes the json, the log, and the media assets folder', () => {
        seedEntry([
            { type: 'image', url: 'file:0.png', mimeType: 'image/png', size: 1, nodeId: '1' }
        ]);
        persistResultAssets(tmpRoot, WORKFLOW_ID, GENERATION_ID, [
        ...
        expect(deleteGenerationFiles(tmpRoot, WORKFLOW_ID, GENERATION_ID)).toBe(true);

        expect(readGenerationFile(tmpRoot, WORKFLOW_ID, GENERATION_ID)).toBeNull();
        expect(fs.existsSync(assetsDir())).toBe(false);
    });

    it('returns false when the generation json does not exist', () => {
        expect(deleteGenerationFiles(tmpRoot, WORKFLOW_ID, GENERATION_ID)).toBe(false);
    });
});

describe('migrateGenerationAssets', () => {
    it('converts inline base64 results to asset files and rewrites the json once', () => {
        ...
        const loaded = readGenerationFile(tmpRoot, WORKFLOW_ID, GENERATION_ID)!;
        const migrated = migrateGenerationAssets(tmpRoot, WORKFLOW_ID, GENERATION_ID, loaded);
        ...
        const stored = readGenerationFile(tmpRoot, WORKFLOW_ID, GENERATION_ID)!;
    ...
    it('is a no-op for generations already holding only file:/remote references', () => {
        seedEntry([
            { type: 'image', url: 'file:0.png', mimeType: 'image/png', size: 1, nodeId: '1' },
